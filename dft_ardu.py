...
with tab3:
    ...
    st.set_page_config(page_title="Transformada de Fourier — Explicação e FFT", layout="wide")
    ...
    "quando lidamos com sinais discretos e finitos, aplicamos a Transformada Discreta de Fourier (DFT), cuja definição "
    "matemática revela uma matriz de exponenciais complexas que traduz amostras no domínio do tempo para coeficientes no domínio das frequências; "
    "a FFT é uma família de algoritmos que computam exatamente a DFT de maneira eficiente, explorando fatorações de N e propriedades de periodicidade das raízes da unidade; "
    "matematicamente, a passagem entre domínios é descrita pela Transformada Contínua: $$\\hat{f}(\\omega)=\\int_{-\\infty}^{\\infty} f(t) e^{-i\\omega t} \,dt$$ e sua inversa $$f(t)=\\frac{1}{2\\pi}\\int_{-\\infty}^{\\infty} \\hat{f}(\\omega)e^{i\\omega t} \,d\\omega$$, e para sinais discretos de comprimento N a DFT é $$X_k=\\sum_{n=0}^{N-1} x_n e^{-2\\pi i kn/N}$$; "
    "na prática, atenção a detalhes como aliasing, amostragem (teorema de Nyquist), janelamento e resolução espectral são cruciais para interpretar corretamente os espectros obtidos."
    )

    st.title("Transformada de Fourier — Histórico, Fórmulas e FFT")
    ...
    st.latex(r"\text{Complexidade FFT: }O(N\log N)\quad\text{(ex.: Cooley--Tukey)}")
    ...
    if boom:
        ...
        fig1 = go.Figure()
        fig1.add_trace(go.Scatter(x=t[:512], y=x[:512], mode='lines', name='Sinal', marker_color='black'))
        fig1.update_layout(
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=freqs_pos, y=np.abs(X_pos) / N, mode='lines', name='Magnitude', marker_color='black'))
        fig2.update_layout(
